...
C       Its default is 1. (for 100% effect), and its range is [0. 1.]
      ...
      _RL SPsalFRAC, SaltPlumeCriterion, SPovershoot, SPModIceThickness
...
C     SPModIceThickness :: Mod for sea ice thickness (m))
...
#ifdef SALT_PLUME_DIC
C Common block to allow salt plume to work with prescribed seaice fraction 
C  from the DIC package
      COMMON /SALT_PLUME_DIC/
     &   SPModSaltIce, SPModIceThickness, SPModRhoIce,
     &   FIce_old, spHeff, spdHeffdt

      _RL FIce_old (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      _RL spHeff   (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      _RL spdHeffdt(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      _RL SPModIceThickness
      _RL SPModRhoIce
      _RL SPModSaltIce
#endif
